...
def preprocess_single_image(img_path, opt):
    ...
    # Charger le fichier JSON contenant les rectangles
    json_file_path = os.path.join(os.path.dirname(out_dir), 'rectangles.json')  # Remplacez par le chemin correct du fichier .json
    img_name=os.path.basename(img_path).split('.')[0]+'.jpg'
    # Charger les données depuis le fichier JSON
    with open(json_file_path, "r") as json_file:
        rectangles_dict = json.load(json_file)
    print(rectangles_dict[img_name])
    rect=rectangles_dict[img_name]
    # carve background
    #print(f'[INFO] background removal...')
    carved_image = rembg.remove(image, session=rembg.new_session('sam'),sam_prompt= [{"type": "rectangle", "data": rect, "label": 1}]) # [H, W, 4]
